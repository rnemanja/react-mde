...
export function getContentLengthOfAllBlocksBefore(editorState, key) {
    ...
    let currentKey = key;
    while (true) {
        blockBefore = editorState
            .getCurrentContent()
            .getBlockBefore(currentKey);
        if (!blockBefore) {
            break;
        }
...
export async function getMdeStateFromDraftState(
    editorState: EditorState,
    generateMarkdownPreview: GenerateMarkdownPreview,
): Promise<MdeState> {
    const markdown = getPlainText(editorState);
    const html = generateMarkdownPreview
        ? await generateMarkdownPreview(markdown)
        : '';
    return {
        html,
        markdown,
...
export async function buildNewMdeState(
    currentState: MdeState,
    generateMarkdownPreview: GenerateMarkdownPreview,
    newText: string,
    newSelection: TextSelection = null,
): Promise<MdeState> {
    const newDraftState = buildNewDraftState(
        currentState.draftEditorState,
        newText,
        newSelection,
    );
    const html = generateMarkdownPreview
        ? await generateMarkdownPreview(newText)
        : '';
    return {
        html,
        markdown: newText,
